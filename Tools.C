...
uint64_t Tools::getByte(uint64_t source, int32_t byteNum)
{
 ...
 mask = (mask << (8 * byteNum));
 uint64_t return_value = (mask & source);
 return_value = (return_value >> (8 * byteNum));
 return return_value;
}
...
uint64_t Tools::getBits(uint64_t source, int32_t low, int32_t high)
{
 ...
 uint64_t mask_low = (mask << low);
 uint64_t mask_high = (mask >> (63 - high));
 mask = mask_low & mask_high;
 uint64_t return_value = mask & source;
 return_value = return_value >> low;
 return return_value;
}
                        ...
                        int32_t srclow, int32_t dstlow, int32_t length)
{
 if (
   !(srclow >= 0 && srclow <= 63) ||
   !(dstlow >= 0 && dstlow <= 63) ||
   !((dstlow + length - 1) >= 0 && (dstlow + length - 1) <= 63)
   )
 {
   return dest;
 }
 uint64_t copy_bits = getBits(source, srclow, srclow + length - 1);
 dest = clearBits(dest, dstlow, dstlow + length - 1);
 copy_bits = (copy_bits << dstlow);
 //return dest;
 return dest + copy_bits;
}
...
uint64_t Tools::setByte(uint64_t source, int32_t byteNum)
{
 ...
 //bit shift to test if changed
 uint64_t mask2 = mask >> 8;
 mask2 = mask2 << 8;
 mask2 = (mask ^ mask2);
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
 ...
 int sign1 = op1 >> 63;
 int sign2 = op2 >> 63;
 int sign3 = ((op2 - op1) >> 63);
 return ((sign1 + sign2 == 1) && (sign1 == sign3));
}
